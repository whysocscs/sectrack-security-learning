...
test('Week 0-15 activities and workloads are available', () => {
  assert.deepEqual(activityTypes, ['exploration', 'lesson', 'practice', 'investigation', 'simulation', 'external', 'report', 'assessment'])
  assert.deepEqual(Object.values(weekContent).map((week) => week.index), Array.from({ length: 16 }, (_, index) => index))
...
test('self explanation, mastery, confidence, and review state stay separate', () => {
  ...
  const afterQuiz = recordQuizAttempt(progress, { weekIndex: 2, answers, attemptedAt: '2026-07-11T01:05:00.000Z' })
