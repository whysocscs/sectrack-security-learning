...
const LabCatalog = React.lazy(() => import('./components/Labs').then((module) => ({ default: module.LabCatalog })))
const LabPage = React.lazy(() => import('./components/Labs').then((module) => ({ default: module.LabPage })))
const ReportsPage = React.lazy(() => import('./components/Reports').then((module) => ({ default: module.ReportsPage })))
...
class RouteErrorBoundary extends React.Component {
  ...
  render() {
    ...
    return (
      <section className="empty-state" role="alert" aria-labelledby="route-error-title">
        <AlertTriangle size={28} aria-hidden="true" />
        <h1 id="route-error-title">이 화면을 표시하지 못했습니다.</h1>
        <p>저장된 학습 기록은 그대로 두었습니다. 로드맵으로 돌아가 다른 항목을 열거나 페이지를 새로고침해 주세요.</p>
        <button type="button" className="primary-button" onClick={this.props.onRecover}>학습 로드맵으로 돌아가기</button>
      </section>
    )
  }
}
...
export default function App() {
  ...
  const activeModule = currentWeek?.modules.find((module) => module.id === route.moduleId)
  const pageTitle = currentWeek
...
function Topbar({ title, subtitle, onMenu, navigate, menuButtonRef, menuOpen, progress, updateProgress, onExport, onImport, routeKey }) {
  const [settingsOpen, setSettingsOpen] = useState(false)
  const settingsRef = useRef(null)
  const settingsButtonRef = useRef(null)
  const importRef = useRef(null)
  const fontOptions = [['100', '보통'], ['112.5', '크게'], ['125', '더 크게'], ['150', '최대']]
  useEffect(() => { setSettingsOpen(false) }, [routeKey])
  useEffect(() => {
    ...
    <header className="topbar">
      ...
      <div className="topbar-actions">
        <div className="accessibility-menu" ref={settingsRef}>
          <button ref={settingsButtonRef} type="button" className="icon-button" aria-label="글자 크기와 데이터 설정" aria-expanded={settingsOpen} onClick={() => setSettingsOpen((value) => !value)}><Type size={18} /></button>
  ...
  return (
    <div className="home-layout page-width">
      ...
      <aside className="home-rail">
        ...
        <section className="rail-section due-list"><h2>evidence-ready 전 주차</h2>{recordableWeeks.filter((item) => getWeeklyRecordState(progress, item.index) !== 'evidence-ready').slice(0, 3).map((item) => <button type="button" key={item.id} onClick={() => navigate({ page: 'week', week: item.index, tab: 'record' })}><span>W{String(item.index).padStart(2, '0')}</span><strong>{item.deliverables[0]}</strong><ChevronRight size={15} /></button>)}</section>
      </aside>
    </div>
  )
}
...
function WeekPage({ week, route, progress, updateProgress, navigate, notify }) {
  ...
  const [guideState, setGuideState] = useState({ weekIndex: null, status: 'idle', modules: null, error: null })
  const tab = route.tab || 'overview'
  useEffect(() => {
    if (tab !== 'concepts' || !supportsDeepGuide(week.index)) return undefined
    let active = true
    setGuideState((current) => current.weekIndex === week.index && current.status === 'ready'
      ? current
      : { weekIndex: week.index, status: 'loading', modules: null, error: null })
    loadDeepGuideModules(week.index, week.modules).then((modules) => {
      if (active) setGuideState({ weekIndex: week.index, status: 'ready', modules, error: null })
    }).catch(() => {
      if (active) setGuideState({ weekIndex: week.index, status: 'error', modules: null, error: '심화 학습 자료를 불러오지 못했습니다.' })
    })
    return () => { active = false }
  }, [guideAttempt, tab, week.index, week.modules])
  ...
  const renderedWeek = guideState.weekIndex === week.index && guideState.status === 'ready' ? { ...week, modules: guideState.modules } : week
  ...
  const completedLabs = requiredLabs.filter((lab) => isActivityRecorded(progress.labs[lab.id])).length
...
function WeekZeroPage({ route, week, progress, updateProgress, navigate, notify }) {
  ...
  const moduleDestinations = {
    ...
    'w0-careers': { tab: 'careers', section: 'roles' },
    'w0-evidence': { tab: 'careers', section: 'postings' },
  }
  ...
  return <div className="page-width week-page week0-page">
    ...
    {tab === 'quiz' ? <QuizView key="week-zero" week={week} progress={progress} updateProgress={updateProgress} navigate={navigate} notify={notify} /> : <WeekZeroWorkspace activeTab={tab} moduleId={route.moduleId} initialCareerSection={moduleDestination?.section} progress={progress} updateProgress={updateProgress} navigate={navigate} notify={notify} onTabChange={openTab} onCompleteMap={() => { updateProgress((current) => ({ ...current, labs: { ...current.labs, 'w0-map': { ...(current.labs['w0-map'] || {}), status: 'activity-recorded', recordedAt: new Date().toISOString() } } })); notify('나의 보안 지도 활동을 기록했습니다. 자동 상태는 사람의 숙련 검토를 뜻하지 않습니다.') }} />}
  </div>
}
...
function ConceptReader({ week, selectedId, sectionId, progress, updateProgress, openModule, openSection }) {
  ...
  useEffect(() => {
    ...
    if (!sectionId || sectionId !== validSectionId) return undefined
  ...
  }, [sectionEntries, sectionId, selected.id])
  useEffect(() => {
    if (typeof IntersectionObserver !== 'function' || sectionEntries.length < 2) return undefined
    ...
    sectionEntries.forEach(({ targetId }) => {
...
function WeekLabs({ week, progress, navigate }) {
  ...
  return <div className="week-lab-groups">{groups.map((group) => <section key={group.id}><header><span>{group.id === 'required' ? 'REQUIRED' : 'EXTENSION'}</span><h2>{group.title}</h2><p>{group.description}</p></header><div className="week-lab-list">{group.labs.map((lab, index) => { const state = progress.labs[lab.id]; const recorded = isActivityRecorded(state); return <button type="button" key={lab.id} onClick={() => navigate({ page: 'lab', labId: lab.id })}><span className="lab-sequence">{String(index + 1).padStart(2, '0')}</span><span className="lab-list-copy"><small>{lab.path === 'extension' ? '심화' : lab.kind === 'external' ? '외부 실습' : '필수 핵심'}</small><strong>{lab.title}</strong><p>{lab.objective}</p><span>{lab.requiredTools.join(' · ')}</span></span><Status text={recorded ? '활동 기록됨' : state ? '진행 중' : '미시작'} tone={recorded ? 'done' : state ? 'progress' : 'muted'} /><ChevronRight size={18} /></button>})}</div></section>)}</div>
}

function QuizView({ week, progress, updateProgress, navigate, notify }) {
  const [quizSeed] = useState(() => progress.quizSeeds?.[week.index]
    || `sectrack-week-${week.index}-${globalThis.crypto?.randomUUID?.() || Date.now()}`)
  const questions = useMemo(() => getQuizPresentation(week.index, quizzes[week.index], quizSeed), [week.index, quizSeed])
  ...
  const submit = () => {
    ...
    updateProgress((current) => ({
      ...recordQuizAttempt(current, { weekIndex: week.index, answers, questions, attemptedAt: new Date().toISOString() }),
      quizSeeds: { ...(current.quizSeeds || {}), [week.index]: quizSeed },
    }))
  ...
  return (
    <section className="quiz-page">
      ...
      {questions.map((question, index) => {
        ...
        const isCore = coreQuestionIds.has(question.id)
        const feedbackId = `${question.id}-feedback`
        return <fieldset className="quiz-card" key={question.id} aria-describedby={submitted ? feedbackId : undefined}><legend><span>Q{index + 1} · {question.difficulty}{isCore && <b className="quiz-core">핵심 문항</b>}</span><strong>{question.question}</strong></legend><div className="quiz-options">{question.options.map((option, optionIndex) => <label key={option.id} className={selectedId === option.id ? 'selected' : ''}><input type="radio" name={question.id} value={option.id} checked={selectedId === option.id} onChange={() => { setAnswers((current) => ({ ...current, [question.id]: option.id })); setSubmitted(false) }} /><i>{String.fromCharCode(65 + optionIndex)}</i><span>{option.label}</span></label>)}</div>{submitted && <div id={feedbackId} className={`quiz-explanation ${correct ? 'correct' : 'wrong'}`}><strong>{correct ? '정답' : '다시 확인'}</strong><p>{selectedOption?.rationale || question.explanation}</p>{!correct && <p className="quiz-correct-model"><b>정답의 핵심 모델:</b> {correctOption?.rationale || question.explanation}</p>}{!correct && <div className="quiz-remediation"><span>복습할 개념</span>{question.remediationModuleIds.map((moduleId) => <button type="button" key={moduleId} onClick={() => navigate({ page: 'week', week: week.index, tab: 'concepts', moduleId })}>{getConceptTitle(moduleId) || '관련 개념 모듈'}<ChevronRight size={14} /></button>)}</div>}</div>}</fieldset>
      })}
      {submitted && <section ref={resultRef} tabIndex="-1" role="status" aria-live="polite" className="quiz-next-action"><div><strong>{result.passed ? '이해 확인을 통과했습니다.' : '아직 통과 기준에 도달하지 않았습니다.'}</strong><p>{result.passed ? '이번 주에 남긴 관찰과 근거를 주차 학습 정리로 연결하세요.' : '선택한 오답의 이유와 연결 모듈을 확인한 뒤 다시 시도하세요.'}</p></div>{result.passed && <button className="button secondary" type="button" onClick={() => navigate({ page: 'week', week: week.index, tab: 'record' })}>주차 정리로<ArrowRight size={16} /></button>}</section>}
...
function SubmissionView({ week, progress, updateProgress, notify }) {
  ...
  useEffect(() => { setForm(progress.evidence[key] || {}); setChecked(progress.evidence[key]?.checked || []); setFieldErrors({}) }, [week.index])
  const update = (field, value) => setForm((current) => ({ ...current, [field]: value }))
  const updateReflection = (field, value) => setForm((current) => ({ ...current, learningReflection: { ...(current.learningReflection || {}), [field]: value } }))
  const validateRecord = () => {
    const errors = {}
    const summaryError = validateLearningText(form.summary, { minLength: 80, label: '학습 요약' })
    const evidenceError = validateLearningText(form.evidence, { minLength: 30, label: '수행 순서와 관찰 결과' })
    if (summaryError) errors.summary = summaryError
    if (evidenceError) errors.evidence = evidenceError
    if (checked.length !== week.deliverables.length) errors.checked = `학습 정리 항목 ${week.deliverables.length}개를 모두 직접 확인하세요. 현재 ${checked.length}개입니다.`
    const reflection = validateStructuredReflection(form.learningReflection || {})
    Object.assign(errors, reflection.fieldErrors)
    return errors
  }
  const requiredModules = week.hideModuleProgress ? [] : week.modules.filter((module) => module.path === 'required')
  const requiredLabs = week.labs.filter((lab) => lab.path === 'required')
  const latestQuiz = progress.quizAttempts?.[week.index]?.at(-1) || progress.quizScores?.[week.index]
  const prerequisites = {
    modules: requiredModules.every((module) => progress.modulesRead[module.id]),
    labs: requiredLabs.every((lab) => isActivityRecorded(progress.labs[lab.id])),
    quiz: Boolean(latestQuiz?.passed),
  }
  const save = (requestEvidenceReady = false) => {
    const errors = requestEvidenceReady ? validateRecord() : {}
    if (requestEvidenceReady && Object.keys(errors).length) {
      setFieldErrors(errors)
      notify('학습 정리의 미충족 항목을 확인하세요.')
      window.requestAnimationFrame(() => errorSummaryRef.current?.focus())
      return
    }
    const evidence = { ...form, checked, updatedAt: new Date().toISOString() }
    const evidenceReady = requestEvidenceReady && Object.values(prerequisites).every(Boolean)
    const status = evidenceReady ? 'evidence-ready' : requestEvidenceReady ? 'activity-recorded' : 'draft'
    updateProgress((current) => ({
      ...current,
      evidence: { ...current.evidence, [key]: evidence },
      submissions: requestEvidenceReady ? { ...current.submissions, [key]: { status, recordedAt: new Date().toISOString(), ...(evidenceReady ? { evidenceReadyAt: new Date().toISOString() } : {}) } } : current.submissions,
    }))
    setFieldErrors({})
    notify(evidenceReady ? '주차 evidence-ready 기준을 충족했습니다. 사람의 검토 승인은 별도입니다.' : requestEvidenceReady ? '기록 구조는 저장했습니다. 읽기·필수 활동·퀴즈 통과 뒤 evidence-ready가 됩니다.' : '학습 정리 초안을 저장했습니다.')
  }
  const submissionState = getWeeklyRecordState(progress, week.index)
  const reflectionFields = [
    ['normalFlow', '정상 동작', '이 기술이나 통제가 정상일 때 주체·데이터·상태가 어떤 순서로 움직이는지 적으세요.'],
    ['trustBoundary', '신뢰 경계', '누가 어떤 값을 신뢰 경계로 넘기고 어느 주체가 판단해야 하는지 적으세요.'],
    ['failurePoint', '실패 지점', '문제가 성립하는 전제와 정확한 코드·설정·권한·설계 지점을 적으세요.'],
    ['patchRemediation', '패치·완화', '근본 수정과 보조 완화가 각각 흐름의 무엇을 바꾸는지 적으세요.'],
    ['retest', '재시험', '실패 경로가 사라지고 정상 기능이 유지되는지 확인할 조건을 적으세요.'],
  ]
  return (
    <div className="submission-layout">
      <section className="submission-form-panel"><header><span>WEEKLY LEARNING RECORD</span><h2>{blueprint?.title || `${week.index}주차 학습 정리`}</h2><p>{blueprint?.description || '결과만 붙이지 말고 어떤 조건에서 무엇을 관찰했고 무엇을 이해했는지 본인의 문장으로 남깁니다.'}</p></header>{Object.keys(fieldErrors).length > 0 && <div className="form-error-summary" role="alert" tabIndex="-1" ref={errorSummaryRef}><strong>저장 전 확인할 항목이 있습니다.</strong><ul>{Object.values(fieldErrors).map((error) => <li key={error}>{error}</li>)}</ul></div>}{blueprint && <section className="record-blueprint"><span>기록 순서</span><ol>{blueprint.sections.map((item, index) => <li key={item}><b>{String(index + 1).padStart(2, '0')}</b>{item}</li>)}</ol></section>}<label><span>학습 요약 <em>필수 · 공백 제외 80자 이상</em></span><textarea required minLength="80" aria-invalid={Boolean(fieldErrors.summary)} aria-describedby={fieldErrors.summary ? `${key}-summary-error` : undefined} rows="6" value={form.summary || ''} onChange={(event) => update('summary', event.target.value)} placeholder="이번 주 핵심 개념과 실습에서 확인한 관계를 본인의 언어로 적으세요." />{fieldErrors.summary && <small id={`${key}-summary-error`} className="field-error">{fieldErrors.summary}</small>}<small>{getLearningTextLength(form.summary)} / 80자</small></label><label><span>수행 순서와 관찰 결과 <em>필수 · 공백 제외 30자 이상</em></span><textarea required minLength="30" aria-invalid={Boolean(fieldErrors.evidence)} aria-describedby={fieldErrors.evidence ? `${key}-evidence-error` : undefined} rows="8" value={form.evidence || ''} onChange={(event) => update('evidence', event.target.value)} placeholder="민감정보를 마스킹한 명령 출력, 요청·응답, 화면 관찰 내용을 적으세요." />{fieldErrors.evidence && <small id={`${key}-evidence-error`} className="field-error">{fieldErrors.evidence}</small>}<small>{getLearningTextLength(form.evidence)} / 30자</small></label><section className="structured-reflection"><h3>핵심 자기 설명 <span>각 항목 공백 제외 20자 이상</span></h3><p>자동 검사는 구조와 명백한 무내용만 확인하며 설명의 정확성을 판정하거나 숙련을 승인하지 않습니다.</p>{reflectionFields.map(([id, label, placeholder]) => <label key={id}><span>{label}<em>필수</em></span><textarea required minLength="20" rows="4" aria-invalid={Boolean(fieldErrors[id])} aria-describedby={fieldErrors[id] ? `${key}-${id}-error` : undefined} value={form.learningReflection?.[id] || ''} onChange={(event) => updateReflection(id, event.target.value)} placeholder={placeholder} />{fieldErrors[id] && <small id={`${key}-${id}-error`} className="field-error">{fieldErrors[id]}</small>}<small>{getLearningTextLength(form.learningReflection?.[id])} / 20자</small></label>)}</section><label><span>막힌 지점과 힌트 사용 <small>선택</small></span><textarea rows="4" value={form.reflection || ''} onChange={(event) => update('reflection', event.target.value)} placeholder="처음 가설, 막힌 지점, 힌트 뒤 바뀐 판단을 적으세요." /></label><label><span>풀이·블로그 URL <small>선택</small></span><input type="url" value={form.url || ''} onChange={(event) => update('url', event.target.value)} placeholder="https://" /></label><footer><button className="button secondary" type="button" onClick={() => save(false)}>초안 저장</button><button className="button primary" type="button" onClick={() => save(true)}>기록 검증·저장<ArrowRight size={16} /></button></footer></section>
      <aside className="submission-checklist"><h3>학습 정리 항목 <small>자기 확인</small></h3>{week.deliverables.map((item, index) => <label key={item}><input type="checkbox" checked={checked.includes(index)} onChange={(event) => setChecked((current) => event.target.checked ? [...current, index] : current.filter((value) => value !== index))} /><span>{item}</span></label>)}{fieldErrors.checked && <p className="field-error">{fieldErrors.checked}</p>}<section className="record-gates"><h3>evidence-ready 조건</h3><ul><li className={prerequisites.modules ? 'met' : ''}>{prerequisites.modules ? '충족' : '미충족'} · 필수 개념 읽기</li><li className={prerequisites.labs ? 'met' : ''}>{prerequisites.labs ? '충족' : '미충족'} · 필수 활동 기록</li><li className={prerequisites.quiz ? 'met' : ''}>{prerequisites.quiz ? '충족' : '미충족'} · 이해 확인 통과</li><li className={submissionState === 'evidence-ready' ? 'met' : ''}>{submissionState === 'evidence-ready' ? '충족' : '기록 검증 필요'} · 구조화 자기 설명</li></ul><p>비선형으로 먼저 작성할 수 있지만 모든 조건 전에는 주차 완료로 표시하지 않습니다.</p></section><div className="submission-rule"><ShieldCheck size={18} /><strong>마스킹 확인</strong><p>비밀번호, 다음 레벨 자격 증명, Cookie, Authorization, API Key, 개인정보는 원문 대신 `[REDACTED]`로 표시합니다.</p></div><Status text={submissionState === 'evidence-ready' ? 'evidence-ready' : submissionState === 'activity-recorded' ? '기록됨 · 조건 대기' : submissionState === 'draft' ? '초안' : '미작성'} tone={submissionState === 'evidence-ready' ? 'done' : submissionState === 'activity-recorded' ? 'progress' : 'muted'} /></aside>
    </div>
  )
}
  ...
  return (
    <div className="page-width resources-page">
      <section className="local-reference sanitized-reference">
        <div><span>REDACTED FOR PUBLIC DEPLOYMENT</span><h2>학생형 XSS 보고서는 구조화된 사례만 공개합니다</h2><p>업로드된 원본 PDF에는 작성자 식별자와 외부 실습 호스트·공격 링크가 포함되어 있어 공개 배포에서 제외했습니다. 화면상의 Week 03 보고서에는 개인 식별 정보와 원본 URL을 제거한 6개 교육 사례만 제공합니다.</p><span className="sanitized-badge"><ShieldCheck size={16} />원본 파일 비공개 · 메타데이터 미배포</span></div>
...
function ProgressPage({ progress, navigate }) {
  const weeks = getAvailableWeeks()
  const recordableWeeks = getRecordableWeeks(weeks)
  const submittedWeeks = recordableWeeks.filter((week) => getWeeklyRecordState(progress, week.index) === 'evidence-ready').length
  ...
  return (
    <div className="page-width progress-page">
      <section className="progress-summary"><div><small>읽기 진행</small><strong>{weeks.flatMap((week) => week.hideModuleProgress ? [] : week.modules).filter((module) => progress.modulesRead[module.id]).length}<span> / {weeks.reduce((sum, week) => sum + (week.hideModuleProgress ? 0 : week.modules.length), 0)}</span></strong></div><div><small>활동 기록</small><strong>{Object.values(progress.labs).filter(isActivityRecorded).length}<span> / {weeks.reduce((sum, week) => sum + week.labs.length, 0)}</span></strong></div><div><small>evidence-ready 주차</small><strong>{submittedWeeks}<span> / {recordableWeeks.length}</span></strong></div></section>
      <section className="document-section"><SectionTitle title="주차별 진행" description="읽기 진행, 활동 기록, quiz-passed, evidence-ready를 별도 상태로 표시합니다. 자동 상태는 숙련 또는 검토 승인을 뜻하지 않습니다." /><div className="progress-week-table"><div><span>주차</span><span>개념</span><span>실습</span><span>이해 확인</span><span>주차 정리</span><span>진행</span></div>{weeks.map((week) => { const breakdown = calculateProgressBreakdown(week, progress); const value = breakdown.percent; const recordState = getWeeklyRecordState(progress, week.index); return <button type="button" key={week.id} onClick={() => navigate({ page: 'week', week: week.index, tab: 'overview' })}><strong>W{String(week.index).padStart(2, '0')} · {week.title}</strong><span>{week.hideModuleProgress ? '탐색형' : `${breakdown.reading.completed}/${breakdown.reading.total}`}</span><span>{breakdown.activityCompletions.completed}/{breakdown.activityCompletions.total}</span><span>{breakdown.understandingCheck.passed ? 'quiz-passed' : breakdown.understandingCheck.attempted ? '재시도 필요' : '미응시'}</span><span>{recordState === 'evidence-ready' ? 'evidence-ready' : recordState === 'activity-recorded' ? '기록됨' : recordState === 'draft' ? '초안' : '미작성'}</span><span><ProgressBar value={value} /><b>{value}%</b></span></button>})}</div></section>
      <section className="document-section"><SectionTitle title="개념 자기평가와 근거" description="읽음·퀴즈 근거·본인의 자기 설명·사람의 검토 승인은 서로 다른 상태입니다." />{masteryEntries.length ? <div className="mastery-list">{masteryEntries.map(([id, level]) => { const evidence = progress.conceptEvidence[id] || {}; return <div key={id}><span><strong>{getConceptTitle(id) || '직무 지도 또는 이전 과정 개념'}</strong><small>퀴즈 근거 {evidence.quizResults?.length || 0}개 · 자기 설명 {String(evidence.selfExplanation?.text || '').trim() ? '기록' : '미기록'} · 확신도 {progress.confidence[id] || progress.mindmap?.confidence?.[id] || '미기록'} · 사람 검토 미연결</small></span><Status text={`자기평가 · ${masteryLabels[level] || level}`} tone="progress" /></div> })}</div> : <EmptyState icon={BookMarked} title="아직 자기평가 근거가 없습니다." text="개념 모듈에서 자기 설명·확신도·현재 수준을 기록하세요. 이 상태는 자동 채점이나 교수자 승인이 아닙니다." />}</section>
    </div>
  )
}
