...
function Checkpoint({ block, value, onChange }) {
  ...
  return (
    <section className="lesson-checkpoint" aria-labelledby={block.id}>
      ...
      {correct !== null && <div className={`checkpoint-result ${correct ? 'correct' : 'retry'}`}><strong>{correct ? '확인됨' : '다시 읽기'}</strong><span>{block.explanation || (correct ? '선택한 답이 이 블록의 핵심과 일치합니다.' : '해설과 앞선 예시를 다시 확인한 뒤 답을 바꿔 보세요.')}</span></div>}
...
function Comparison({ block }) {
  ...
  return <section className="lesson-comparison"><BlockHeading block={block} /><div tabIndex="0" aria-label={`${block.title || '비교'} 표, 가로로 스크롤할 수 있습니다.`}><table><thead><tr>{columns.map((column) => <th key={column} scope="col">{column}</th>)}</tr></thead><tbody>{(block.rows || []).map((row, index) => <tr key={`${row.join('-')}-${index}`}>{row.map((cell, cellIndex) => <td key={`${cell}-${cellIndex}`}><InlineCodeText text={cell} /></td>)}</tr>)}</tbody></table></div></section>
}
...
export default function LessonRenderer({ module, checkpointResults = {}, onCheckpoint, onOpenLab }) {
  ...
  return <div className="lesson-blocks">{blocks.map((block, index) => {
    ...
    if (block.type === 'misconception') return <section id={id} key={id} className="lesson-misconception"><BlockHeading block={block} /><ul>{(block.items || []).map((item) => <li key={item}><span>오해</span><InlineCodeText text={item} /></li>)}</ul></section>
    ...
    if (block.type === 'practice-link') return <section id={id} key={id} className="lesson-practice-links"><BlockHeading block={block} /><p>{block.body || '이 개념을 읽은 뒤에는 관찰 결과를 직접 기록해 보세요.'}</p><div>{(block.labIds || []).map((labId) => <button type="button" key={labId} onClick={() => onOpenLab?.(labId)}>실습 열기 · {labId}</button>)}</div></section>
