  ...
  return (
    <div className={`${fullPage ? 'page-width ' : ''}mindmap-page`}>
      ...
      <section className="career-explorer">
        <header className="career-explorer-header"><div><span>CAREER EVIDENCE EXPLORER</span><h2>직무를 연결하고, 근거의 한계를 함께 읽기</h2><p>Graph View는 모든 직무군과 세부 역할을 보여 줍니다. 역할을 선택하면 필요한 개념·기술·표준 연결이 추가됩니다.</p></div><div className="career-explorer-tabs" role="group" aria-label="직무 탐색 보기"><button type="button" aria-pressed={careerView === 'graph'} className={careerView === 'graph' ? 'active' : ''} onClick={() => setCareerView('graph')}>Graph View</button><button type="button" aria-pressed={careerView === 'evidence'} className={careerView === 'evidence' ? 'active' : ''} onClick={() => setCareerView('evidence')}>Evidence View</button><button type="button" aria-pressed={careerView === 'study'} className={careerView === 'study' ? 'active' : ''} onClick={() => setCareerView('study')}>Study Map</button></div></header>
